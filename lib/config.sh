...
load_config() {
  ...
  DELETE_REMOTE=$(jq -r '.deleteRemote // false' "$CONFIG_FILE" 2>/dev/null)
...
OPTIONS:
  ...
  --delete-remote, -R     Also delete the corresponding branch on the remote for each deleted local branch
...
CONFIG:
  ...
  deleteRemote: true/false  - Also delete the corresponding branch on the remote (default: false)
...
JSON
  ...
  cur_deleteRemote=$(jq -r '.deleteRemote // false' "$CONFIG_FILE" 2>/dev/null || echo false)
  ...
  read -rp "Delete remote branches? (true/false) [${cur_deleteRemote}]: " new_deleteRemote
